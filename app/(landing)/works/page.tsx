...
export default function WorksPage() {
  ...
  const buildTimeline = () => {
    ...
    const suwaWidth = el.getBoundingClientRect().width;
    const jumpHeight = window.innerHeight * 0.25;
     const isMobileNow = window.innerWidth <= 1024;
    const duration = isMobileNow ? 10 : 15;
    ...
    const jumpDown = () =>
      gsap
        .timeline()
        .to(el, { y: 0, scaleY: 1.05, scaleX: 0.65, duration: 1.5, ease: "power2.in" })
        .to(el, { scaleY: 0.55, scaleX: 1.15, duration: 0.15, ease: "power2.out" })
        .to(el, { scaleX: 1, scaleY: 1, duration: 0.4, ease: "power1.out" });

    const addJump = (tl: gsap.core.Timeline, startTime: number) => {
      const jumpTl = gsap.timeline();
      jumpTl
        .to(el, { scaleY: 0.65, scaleX: 1.45, duration: 0.2, ease: "power1.in" })
        .to(el, { y: -jumpHeight, scaleY: 1.12, scaleX: 0.9, duration: 0.45, ease: "power2.out" })
        .to(el, { y: 0, scaleY: 1.05, scaleX: 0.7, duration: 0.75, ease: "power2.in" })
        .to(el, { scaleY: 0.6, scaleX: 1.15, duration: 0.15, ease: "power2.out" })
        .to(el, { scaleX: 1, scaleY: 1, duration: 0.35, ease: "power1.out" });
      tl.add(jumpTl, startTime);
    };

    tl = gsap.timeline({
      repeat: -1,
      defaults: { ease: "none" },
      onRepeat: () => buildTimeline(),
    });

    tl.fromTo(el, { x: screenWidth }, { x: -suwaWidth, duration });
    tl.add(jumpDown(), 0);
  ...
  const handleExplosion = () => {
    ...
    setTimeout(() => fadeOutEffect(), 150);

    explosion.addEventListener("complete", () => {
      explosion.destroy();
      gsap.set(el, { autoAlpha: 0 });
      setTimeout(() => router.push("/swipe-drama"), 800);
    });
  };

  el.addEventListener("click", handleExplosion, { once: true });
};
